...
import { TablePagination, Grid, Button, withStyles, CircularProgress } from '@material-ui/core';
...
class DataGrid extends Component {
  ...
  createLeftButtons = () => {
    ...
    if (this.props.leftButtonLabel) {
      return (
        <Button onClick={this.props.leftButtonOnClick} {...this.props.leftButtonProps}>
  ...
  renderAction = action => <Button key={`odg_action_${action.label.replace(' ', '')}`}>{action.label}</Button>;
  ...
  renderNoData() {
    return (
      <Grid container>
        <Grid item xs={12} style={{ textAlign: 'center' }} colSpan={Object.keys(this.props.columns).length}>
          No Data
        </Grid>
      </Grid>
    );
  ...
  render() {
    return (
      <Grid container>
        <Grid item xs={12}>
          {this.props.createForm ? (
            ...
            <CreateForm
              ...
              onClose={() => {
                this.setState({ open: false, model: null });
              }}
              onSubmit={data => {
          ...
          <Grid container className={this.props.tableClassName}>
            {this.props.columns ? (
              <Header
                ...
                actionColumns={this.countActionColumns()}
                showActionColumn={this.props.showEdit || this.props.showDelete}
              />
            ) : null}
        ...
        {this.props.data && this.props.data.length && this.props.showPagination ? this.renderPagination() : null}
      </Grid>
    );
  }
}
